...
void Draw_Prism(float x, float y) {
    glPushMatrix();
...
void Init_Light()
{
 ...
 GLfloat light_position[] = { 0.0f, 0.0f, 20.0f, 1.0f }; //позиция источника
                   ...
                   int nCmdShow)
{
    ...
    while (!bQuit)
    {
        ...
        else
        {
            ...
            float radius = 3.0f;
            int num_prisms = 8;
            float angle_increment = 360.0f / num_prisms;
            float alpha_step = 1.0f / num_prisms;
            glEnable(GL_BLEND);
            glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
            float alpha = 0.1f;
            for (int i = 0; i < num_prisms; ++i) {
            float angle = i * angle_increment * (M_PI / 180);
